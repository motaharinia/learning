 ...
 * A simple basic Java Configuration is given below.
 ...
 * گام 1
 * اولین قدم ایجاد پیکربندی Spring Security Java است.
 *   پیکربندی ساده  جاوا در زیر آورده شده است.
 * این پیکربندی یک فیلتر Servlet ایجاد می کند که با نام SpringSecurityFilterChain شناخته می شود.
 * این مسئول حفاظت از آدرس های اینترنتی برنامه ، تأیید اعتبار نام کاربری و رمز عبور ، هدایت مجدد به فرم ورود به سیستم و غیره است.
 * پیکربندی جاوا فوق برای برنامه ما موارد زیر را انجام می دهد.
 * برای هر URL نیاز به تأیید اعتبار دارید
 * یک فرم ورود ایجاد می کند
 * به کاربر اجازه دهید با استفاده از تأیید اعتبار مبتنی بر فرم تأیید اعتبار کند
 * اجازه ورود به سیستم
 * از حمله CSRF جلوگیری کنید
 * ادغام هدر امنیتی و غیره
 *
 *
 *
...
@EnableWebSecurity
@ComponentScan(basePackages = {"ir.micser"})
@EnableGlobalMethodSecurity(prePostEnabled=true) //spring-security-at-method-level
public class WebSecurityConfig implements WebMvcConfigurer {
    ...
    public UserDetailsService userDetailsService() throws Exception {
        ...
        manager.createUser(User.withDefaultPasswordEncoder()
                .username("irfan")
                .password("user123").roles("USER").build());
        manager.createUser(User.withDefaultPasswordEncoder()
                .username("admin")
